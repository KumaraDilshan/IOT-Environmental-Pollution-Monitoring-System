...
//#define DHT11_PIN A0 //no ; here. Set equal to channel sensor is on
//dht DHT;
...
void loop() {
 ...
 /* int chk = DHT.read11(ANALOGPIN);
  Serial.print("Temperature = ");
  Serial.println(DHT.temperature);
  Serial.print("Humidity = ");
  Serial.println(DHT.humidity);*/
   float temp_f = dht.readTemperature(false);
  ...
  if (! temperaturem.publish(temperature)) {
    Serial.println(F("Failed"));
  } else {
    Serial.println(F("OK!"));
  }
  ...
  if (LPG>20 || CO>20 || smoke>12 || temperature>32 || humidity<50 || dBvalue>85 || CO2>65)
  {
